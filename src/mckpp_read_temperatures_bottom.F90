  ...
  SUBROUTINE mckpp_read_temperatures_bottom()
    ...
    IF (l_root) THEN 
      ...
      CALL mckpp_get_update_time( & 
        file, time, kpp_const_fields%ndtupdbottom, &
      ...
      WRITE(message,*) 'Reading climatological bottom temp for time ', & 
                        update_time
    ...
    END IF 
    CALL mckpp_scatter_field( var_global, kpp_3d_fields%bottom_temp, root )
