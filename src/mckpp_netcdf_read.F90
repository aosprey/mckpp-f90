...
CONTAINS
  ...
  ! Read variable - 4D real
  ! Option to specify start, derive count from array size.
  SUBROUTINE mckpp_netcdf_get_var_real_4d(calling_routine, file_name, &
      ncid, var_name, array, start_in)

    CHARACTER(LEN=*), INTENT(IN) :: calling_routine, file_name, var_name
    INTEGER, INTENT(IN) :: ncid
    REAL, DIMENSION(:,:,:,:), INTENT(OUT) :: array
    INTEGER, DIMENSION(4), INTENT(IN), OPTIONAL :: start_in

    INTEGER :: varid
    INTEGER, DIMENSION(4) :: start, count
    CHARACTER(LEN=max_message_len) :: context, message
    CHARACTER(LEN=28) :: routine = "MCKPP_NETCDF_GET_VAR_REAL_4D"
  ...
  SUBROUTINE mckpp_netcdf_get_var_real_to_2d(calling_routine, file_name, &
    ...
    INTEGER :: varid
    CHARACTER(LEN=max_message_len) :: context, message
    CHARACTER(LEN=31) :: routine = "MCKPP_NETCDF_GET_VAR_REAL_TO_2D"
