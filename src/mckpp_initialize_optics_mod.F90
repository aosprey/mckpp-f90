...
CONTAINS
  ...
  SUBROUTINE mckpp_initialize_optics()
    ...
    INTEGER, DIMENSION(npts) :: jerlov_global
    ...
    IF (kpp_const_fields%l_jerlov) THEN 
      ...
      IF (l_root) THEN 
        ...
        WRITE(message,*) "Reading optical properties of seawater from ", & 
                         TRIM(file)  
        ...
        CALL mckpp_netcdf_get_var( routine, file, ncid, "jerlov", & 
                                   jerlov_global, start, count, 2 )
        CALL mckpp_netcdf_close(routine, file, ncid)

      END IF 
    
      CALL mckpp_scatter_field( jerlov_global, kpp_3d_fields%jerlov, root )
