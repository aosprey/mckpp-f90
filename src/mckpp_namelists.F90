MODULE mckpp_namelists
  ...
  ! name_constants
  REAL :: grav, vonk, sbc, twopi, onepi, TK0, spd, dpy, epsw, albocn, sice, & 
      EL, SL, FL, FLSN
  ...
  LOGICAL :: L_COUPLE, L_CLIMSST, L_UPD_CLIMSST, L_CPLWGHT, L_CLIMICE, & 
      L_UPD_CLIMICE, L_CLIM_ICE_DEPTH, L_CLIM_SNOW_ON_ICE, L_OUTKELVIN, & 
      L_COUPLE_CURRENTS, L_CLIMCURR, L_UPD_CLIMCURR, L_PERIODIC_CLIMICE, & 
      L_PERIODIC_CLIMSST, L_BAD_ICE_DEPTH      
  INTEGER :: ifirst, ilast, jfirst, jlast, ndtupdsst, ndtupdice, ndtupdcurr, &
      climsst_period, climice_period
  CHARACTER(LEN=max_nc_filename_len) :: sstin_file = "", cplwght_file = "", & 
      icein_file = "", currin_file = ""
  NAMELIST/name_couple/ L_COUPLE, ifirst, ilast, jfirst, jlast, L_CLIMSST, & 
      sstin_file, L_UPD_CLIMSST, ndtupdsst, L_CPLWGHT, cplwght_file, & 
      icein_file, L_CLIMICE, L_UPD_CLIMICE, ndtupdice, L_CLIM_ICE_DEPTH, & 
      L_CLIM_SNOW_ON_ICE, L_OUTKELVIN, L_COUPLE_CURRENTS, currin_file, & 
  ...
  ! name_forcing
  LOGICAL :: L_FLUXDATA, L_FCORR_WITHZ, L_VARY_BOTTOM_TEMP, L_FCORR, & 
      L_UPD_FCORR, L_UPD_BOTTOM_TEMP, L_REST, L_PERIODIC_FCORR, & 
      L_PERIODIC_BOTTOM_TEMP, L_SFCORR_WITHZ, L_SFCORR, L_UPD_SFCORR, & 
      L_PERIODIC_SFCORR, L_UPD_SAL, L_PERIODIC_SAL, L_UPD_OCNT, & 
      L_PERIODIC_OCNT, L_NO_FREEZE, L_NO_ISOTHERM, L_DAMP_CURR, & 
      L_INTERP_OCNT, L_INTERP_SAL
  INTEGER :: ndtupdfcorr, ndtupdbottom, fcorr_period, ndtupdsfcorr, & 
      sfcorr_period, bottom_temp_period, sal_period, ndtupdsal, ocnt_period, & 
      ndtupdocnt, isotherm_bottom, dtuvdamp, ndt_interp_ocnt, ndt_interp_sal
  REAL :: isotherm_threshold
  CHARACTER (LEN=max_nc_filename_len) :: forcing_file = "", & 
      fcorrin_file = "", bottomin_file = "", sfcorrin_file = "", & 
      sal_file = "", ocnt_file = ""
  NAMELIST/name_forcing/ L_FLUXDATA, forcing_file, L_FCORR_WITHZ, & 
      ...
      isotherm_threshold, L_DAMP_CURR, dtuvdamp, L_INTERP_OCNT, & 
      ndt_interp_ocnt, L_INTERP_SAL, ndt_interp_sal
  ...
  ! name_parameters
  ! These variables are defined in mckpp_parameters as they are used 
  ! throughout the code. 
  NAMELIST/name_parameters/ nz, ndim, nx, ny, nvel, nsclr, nsb, itermax, & 
      hmixtolfrac, ngrid, nzl, nzu, nzdivmax, nztmax, igridmax, nsflxs, & 
      njdt, ndharm, maxmodeadv, mr, nx_globe, ny_globe
  ...
  NAMELIST/name_procswit/ LKPP, LRI, LDD, LICE, LBIO, LNBFLX, LTGRID, LRHS, & 
      L_SSref
  ...
  NAMELIST/name_start/ L_INITDATA, initdata_file, L_INTERPINIT, L_RESTART, & 
      restart_infile
