...
MODULE mckpp_xios_control
  ...
  USE mckpp_time_control, ONLY: mckpp_get_time, ntime
  USE mckpp_timer, ONLY: mckpp_start_timer, mckpp_stop_timer
  USE mckpp_xios_io, ONLY: mckpp_xios_diagnostic_definition, &
       mckpp_xios_diagnostic_output, mckpp_xios_restart_definition, &
       mckpp_xios_write_restart, mckpp_xios_read_restart
...
CONTAINS 
  ...
  SUBROUTINE mckpp_restart_control() 
    ...
    IF (MOD(ntime, kpp_const_fields%ndt_per_restart) .EQ. 0 .OR. &
        ntime .EQ. kpp_const_fields%num_timesteps) THEN

      ! Set correct time for validity of restart file
      ! (end of this timestep = start of next timestep)
      restart_time = mckpp_get_time(ntime+1)
