  ...
  SUBROUTINE mckpp_read_salinity_3d()
    ...
    IF (l_root) THEN 
      ...
      CALL mckpp_get_update_time( & 
        file, time, kpp_const_fields%ndtupdsal, file_times, num_times, & 
        kpp_const_fields%L_PERIODIC_SAL, kpp_const_fields%sal_period, &
    ...
    END IF 
    CALL mckpp_scatter_field( var_global, kpp_3d_fields%sal_clim, root )
