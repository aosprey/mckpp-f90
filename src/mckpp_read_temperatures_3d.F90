...
CONTAINS
  ...
  SUBROUTINE initialize_temperatures(file, ncid)
    ...
    IF (nz_in .NE. nzp1) THEN
      WRITE(message,*) "Ocean temperature climatology file does not have ", & 
                       "the correct number of vertical levels."
  ...
  SUBROUTINE mckpp_read_temperatures_3d()
    ...
    CALL mckpp_get_update_time( & 
      file, kpp_const_fields%time, kpp_const_fields%ndtupdocnT, file_times, & 
